...
CREATE TABLE `sdh_culture_medium` (
  `IDX` bigint(20) NOT NULL AUTO_INCREMENT,
  `DEVICE_IDX` bigint(20) NOT NULL,
  `DAYKEY` int(11) NOT NULL DEFAULT 0,
  `SENSING_DT` datetime NOT NULL DEFAULT sysdate(),
  `CREATE_DT` datetime NOT NULL,
  `AM_PM_TYPE` varchar(255) DEFAULT NULL,
  `SUNRISE_SUNSET_FLAG` tinyint(4) DEFAULT NULL,
  `SCM_TEMP` double DEFAULT 0,
  `SCM_MOISTURE_CONTENT` double DEFAULT 0,
  `SCM_EC` double DEFAULT 0,
  `SCM_PH` double DEFAULT 0,
  `SCM_HUM` double DEFAULT 0,
  `SCM_XEC` double DEFAULT 0,
  `PORE_EC` double DEFAULT NULL,
  `INTAKE_EC` double DEFAULT NULL,
  PRIMARY KEY (`IDX`),
  KEY `DAYKEY` (`DAYKEY`),
  KEY `DEVICE_IDX` (`DEVICE_IDX`),
  KEY `SENSING_DT` (`SENSING_DT`),
  KEY `idx1_sdh_culture_medium` (`SENSING_DT`,`DEVICE_IDX`),
  KEY `idx2_sdh_culture_medium` (`DAYKEY`,`DEVICE_IDX`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;